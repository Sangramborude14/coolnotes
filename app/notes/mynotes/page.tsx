            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {filteredNotes.map((note: any) => (
                        <div key={note.noteId} className="bg-white/5 rounded-3xl p-8 flex flex-col h-96 backdrop-blur-xl shadow-2xl border border-white/10 hover:border-white/20 transition-all group">
                            ...
                            <div className="flex justify-between items-center pt-6 mt-6 border-t border-white/5">
                                <Link href={`/notes/${note.noteId}`} className="p-3 rounded-full bg-white/5 hover:bg-green-500/20 hover:text-green-400 transition-all">
                                    <Eye size={22} />
                                </Link>
                                <Link href={`/notes/edit/${note.noteId}`} className="p-3 rounded-full bg-white/5 hover:bg-yellow-500/20 hover:text-yellow-400 transition-all">
                                    <Pencil size={22} />
                                </Link>
                                <button onClick={() => handleDelete(note.noteId)} className="p-3 rounded-full bg-white/5 hover:bg-red-500/20 hover:text-red-500 transition-all">
                                    <Trash2 size={22} />
                                </button>
                            </div>
                        </div>
